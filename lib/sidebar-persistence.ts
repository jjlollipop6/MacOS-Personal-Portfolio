...
// Path persistence for full navigation path (e.g., /Users/jmcfadden/Projects/repo/folder)
...
const DISNEY_VIEWS = ["home", "search", "watchlist", "movies", "series", "originals", "detail"] as const;
type DisneyViewPersisted = (typeof DISNEY_VIEWS)[number];

interface DisneyState {
  view: DisneyViewPersisted;
  titleId: string | null;
  brandFilter?: string;
}

export function loadDisneyState(): DisneyState {
  const defaultState: DisneyState = { view: "home", titleId: null };
  if (typeof window === "undefined") return defaultState;
  try {
    const saved = sessionStorage.getItem(STORAGE_KEYS.DISNEY_STATE);
    if (!saved) return defaultState;
    const parsed = JSON.parse(saved);
    const view: DisneyViewPersisted = DISNEY_VIEWS.includes(parsed.view) ? parsed.view : "home";
    return {
      view,
      titleId: typeof parsed.titleId === "string" ? parsed.titleId : null,
      brandFilter: typeof parsed.brandFilter === "string" ? parsed.brandFilter : undefined,
    };
  } catch {
    return defaultState;
  }
...
export function saveDisneyState(
  view: string,
  titleId: string | null,
  brandFilter?: string
): void {
  if (typeof window === "undefined") return;
  try {
    sessionStorage.setItem(STORAGE_KEYS.DISNEY_STATE, JSON.stringify({ view, titleId, brandFilter }));
  } catch {
    // Ignore storage errors
  }
}

export function loadDisneyWatchlist(): string[] {
  if (typeof window === "undefined") return [];
  try {
    const saved = localStorage.getItem(STORAGE_KEYS.DISNEY_WATCHLIST);
    if (!saved) return [];
    const parsed = JSON.parse(saved);
...
export function saveDisneyWatchlist(ids: string[]): void {
  if (typeof window === "undefined") return;
  try {
    localStorage.setItem(STORAGE_KEYS.DISNEY_WATCHLIST, JSON.stringify(ids));
  } catch {
    // Ignore storage errors
  }
}

export function clearDisneyState(): void {
  if (typeof window === "undefined") return;
  try {
    sessionStorage.removeItem(STORAGE_KEYS.DISNEY_STATE);
  } catch {
    // Ignore storage errors
  }
}
