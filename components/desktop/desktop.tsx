...
import { DesktopFileIcon } from "./desktop-file-icon";
...
const CanvasApp = dynamic(() => import("@/components/apps/canvas/canvas-app").then(m => ({ default: m.CanvasApp })));
const SafariApp = dynamic(() => import("@/components/apps/safari/safari-app").then(m => ({ default: m.SafariApp })));
const DisneyApp = dynamic(() => import("@/components/apps/disney/disney-app").then(m => ({ default: m.DisneyApp })));
...
function DesktopContent({
  ...
  const handleDesktopMouseDown = useCallback((e: React.MouseEvent<HTMLDivElement>) => {
    ...
    const onMove = (me: MouseEvent) => {
      ...
      const x = Math.min(sx, me.clientX);
      const y = Math.min(sy, me.clientY);
      const w = Math.abs(me.clientX - sx);
      const h = Math.abs(me.clientY - sy);
  ...
  return (
    <div className="fixed inset-0" data-shell="desktop" onMouseDown={handleDesktopMouseDown}>
      <Image
      ...
      {selectionBox && selectionBox.w > 3 && (
        <div
          ...
          style={{
            ...
            border: "1px solid rgba(99,155,255,0.7)",
            borderRadius: 3,
          }}
        />
      )}
      <DesktopFileIcon
        label="James B. McFadden"
        onOpen={() => handleOpenPreviewFile("resume.pdf", "/resume.pdf", "pdf")}
      />
